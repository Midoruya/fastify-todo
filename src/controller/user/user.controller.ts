...
export default async function (app: FastifyInstance) {
  app.post(
    "/auth",
    { schema: { body: AuthUserSchema } },
    async (
      request: FastifyRequest<{ Body: Static<typeof AuthUserSchema> }>,
      response
    ) => {
      const payload = request.body;
      const getUserData = await app.prisma.user.findFirst({
  ...
  app.post(
    ...
    async (
      request: FastifyRequest<{ Body: Static<typeof RegisterUserSchema> }>,
      response
    ) => {
      const payload = request.body;
      const isHasAccount = await app.prisma.user.findFirst({
        where: { name: payload.username },
      });
      if (isHasAccount) {
        response.status(400);
